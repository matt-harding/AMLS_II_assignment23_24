...
class WhaleDataset(Dataset):
    def __init__(self, csv_file, image_dir):
        self.data = pd.read_csv(
            csv_file
        )  # Pandas dataframe with training label data. Assumes csv file has columns image, species and individual_id
        ...
        }  # dictionary that maps each unique individual ID to a numerical index for one-hot encoding
    ...
    def __getitem__(self, idx):
        ...
        )  # constructs the full file path of the image
        image = Image.open(image_name).convert(
            "RGB"
        )  # opens the image file using PIL's Image.open method and converts it to the RGB color mode
        label = self.encode[
            self.labels[idx]
        ]  # retrieved one-hot encoding for label value
        if self.transform:
